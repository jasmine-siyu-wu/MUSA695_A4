...
asia_busiest_airports <- read.csv("Data/asia_busiest_airports.csv") %>% 
                            merge(airports, .,
...
[Busiest airports in Asia, weekly August 2023 flights](https://simpleflying.com/asia-busiest-airports-august-2023/). Chengdu Tianfu International Airport opened in 2021 was the 8th busiest airport in Asia, August 2023.
...
ggplot() +
  geom_sf(data = asia,
          fill = "#ffffff", color="#333333", linewidth = 0.1) +
  geom_sf(data = plot_data, 
          aes(size=weekly_flights), alpha=0.7,
          col = "#EC7E00") +
  scale_size(breaks = waiver(), range=c(2, 7),
  ...
  #                    label = "Miami International Airport \nInternaional flights: 43.10%"), size = 3,
  #                    fill = NA, label.color = NA, color="#FB8500") +
  labs(title = "<span style='color:#EC7E00'>Chengdu Tianfu International Airport</span> <span style='color:#ffffff'>opened in 2021 was the 8th busiest Asian airport in 2023</span>", 
...
```{r flow map, include=TRUE, cache = FALSE, message = FALSE, warning = FALSE, fig.height=6, fig.width=10}
...
## Day 24: OpenStreetMap 


```{r black white map, include=TRUE, cache = FALSE, message = FALSE, warning = FALSE, fig.height=8, fig.width=9}
...
## Day 26: Minimal map [Pittsburgh bridges by age] 
...
pitts_bridges <- st_read("./Data/National_Bridge_Inventory.geojson") %>% 
  filter(state_code_001 == 42 & county_code_003 == 003 & place_code_004 == 61000)

pitts_map <- get_map(pitts_bb, maptype = "watercolor")

ggplot() +
  geom_sf(data = pitts_streets$osm_lines, inherit.aes = FALSE, 
          color = "#ffffff", size = .5, alpha = .6) +
  geom_sf(data = pitts_small_streets$osm_lines, inherit.aes = FALSE, 
          color = "#8a8a8a", size = .1, alpha = .4) +
  geom_sf(data = pitts_bridges, aes(color = ) inherit.aes = FALSE, 
          color = "#EC7E00") +
  # geom_sf(data = pitts_rivers$osm_lines, inherit.aes = FALSE, color = "#aaaaaa", linewidth=4, alpha = .4) +
  labs(title = "Steel City, aka Pittsburgh, is loaded with bridge history",
