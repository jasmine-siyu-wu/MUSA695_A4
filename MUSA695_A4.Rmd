...
## Day 9: Hexagons map [Job density of Pittsburgh]
...
ggplot(data=plot_data[which(plot_data$S000>5),], aes(x=h_bg_lon, y=h_bg_lat))+
  ...
  geom_segment(data=plot_data[which(plot_data$S000>5),], 
               ...
               linewidth=0.3, col="#ffffff", show.legend = FALSE,
...
## Day 24: Black & White map [Global entry to United States]
...
```{r black white map, include=TRUE, cache = FALSE, message = FALSE, warning = FALSE, fig.height=6, fig.width=9}


ggplot() +
  geom_sf(data = countries,
          fill = "transparent", color="#222222", linewidth = 0.1) +
  ...
  #                    label = "Miami International Airport \nInternaional flights: 43.10%"), size = 3,
  #                    fill = NA, label.color = NA, color="#FB8500") +
  labs(title = "Global entry airports to United States", 
...
## Day 29: Population map [Population density in UK]
